...
function Update() {
    ...
    if (grounded) {
		...
		if(canRun && state == 0){
			if(Input.GetButton("Run") && Input.GetKey("z") && !Input.GetButton("Fire2")){
				run = true;
			}else{
				run = false;
			}
        ...
        if (sliding){
            ...
            moveDirection = Vector3(hitNormal.x, -hitNormal.y, hitNormal.z * inputX);
		...
		if(airControl){
		    ...
		    if(inputY < 0){
		        moveDirection.x = inputX * speed / airControlSpeed;
		        moveDirection.z = inputY * speed / airControlSpeed;
		    }
